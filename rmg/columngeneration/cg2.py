...
class CG2(CG):
    ...
    def __build_column_model(self,machine):
        ...
        omikron = np.ones(nserv,dtype=np.float64) # duais de serviço migrado
        ...
        self._mach[machine]._g = self._mach[machine].model.addVars(nserv,
                                                                   vtype=GRB.BINARY,name="g",obj=-omikron)
    ...
    def compute_column(self,machine, pi, mu, sigma, omikron):
        self.__compute_column(machine, pi, mu, sigma, omikron)

    def __compute_column(self,machine, pi, mu, sigma, omikron):
        ...
        for s in range(nserv):
            ...
            self._mach[machine]._g[s].Obj = -omikron[s]
        ...
        serv = np.array(
            [1* (self._mach[machine]._s[s].X > .5) for s in range(serv)], dtype=np.int32
        )
        g = np.array(
            [1* (self._mach[machine]._g[s].X > .5) for s in range(serv)], dtype=np.int32
        )

        return Column(rc=self._mach[machine].model.objVal,
                      obj=round(self._mach[machine].model._obj.X),
                      procs=q,
                      serv=serv,
                      g=g)
