...
try:
    opts, args = getopt.getopt(sys.argv[1:],"ctqsn:m:a:o:h",["tex","quiet","save","name=","model=","assign=","output=","help"])
...
b = mdl.addVars(nmach,nres,nres,vtype=GRB.INTEGER,name="b",lb=0)

if verbose: print("creating o",flush=True)
o = mdl.addVars(nserv,len(L),vtype=GRB.BINARY,name="o",lb=0)
...
loadcost = mdl.addVars(nres,name="locadcost",vtype=GRB.INTEGER,lb=0,obj=Wlc)
balancecost= mdl.addVars(nres,nres,name="balancecost",vtype=GRB.INTEGER,lb=0,obj=Wbal)
pmc = mdl.addVar(name="pmc",vtype=GRB.INTEGER,lb=0,obj=WPMC)
smc = mdl.addVar(name="smc",vtype=GRB.INTEGER,lb=0,obj=WSMC)
...
if tex:
    ...
    if checker:
        import subprocess
        output = subprocess.check_output(["./checker", modelfile,assignfile,outputfile]).decode("utf-8")
    else:
        output = "não verificado"
    print("\\verb|%s| & %0.3f & %0.3f & %0.3f & %0.3f & %0.3f & %d & %0.3f & %d & %0.3f  & %s \\\\" %
          (name, all_time, load_time, mdl_vars, mdl_constrs, mdl_opt,resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024 +1 , mdl._objbnd, mdl.objVal,  (abs(mdl._objbnd-mdl.objVal)/abs(mdl._objbnd))*100 ,output))
