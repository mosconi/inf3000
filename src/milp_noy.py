...
if verbose: print("constr. t[i,j]=sum(z-[p,i]*z+[p,j])")
mdl.addConstrs((t[i,j]==quicksum(z_minus[p,i]*z_plus[p,j] for p in range(nproc)) for i in range(nmach) for j in range(nmach)),name="t")
