...
if verbose: print("creating z+ vars",flush=True)
z_plus = mdl.addVars(nproc,nmach,name="z+",vtype=GRB.BINARY)
for p in range(nproc):
    for m in range(nmach):
        z_minus[p,m].start=0
        z_plus[p,m].start=0

if verbose: print("creating y",flush=True)
y = mdl.addVars(nproc,nmach,nmach,vtype=GRB.BINARY,name="y")
...
for r in range(nres):
    if T[r]:
        transient = mdl.addConstrs((quicksum(x[p,m]*R[p,r] for p in range(nproc)) + quicksum(z_minus[p,m]*R[p,r] for p in range(nproc)) <= C[m,r] for m in range(nmach)),name="transient")
        for constr in transient:
            transient[constr].setAttr("Lazy", 1)
...
conflito=mdl.addConstrs((quicksum(x[p,m] for p in S[s])<=1 for s in range(len(S)) for m in range(nmach)), name="conflito")
for constr in conflito:
    conflito[constr].setAttr("Lazy", 1 )

if verbose: print("constr. 2*y[p,i,j] >= z_minus[pi] + z_plus[pj]")
mdl.addConstrs((2*y[p,i,j] >= z_minus[p,i] + z_plus[p,j] for p in range(nproc) for i in range(nmach) for j in range(nmach)), name="y")

if verbose: print("constr. t[i,j]=sum(y[p,i,j] in p)")
...
spread= mdl.addConstrs((quicksum(o[s,l] for l in range(len(L))) >= delta[s] for s in range(nserv)),name="serv_spread")
for constr in spread:
    spread[constr].setAttr("Lazy", 1)
...
for s in range(nserv):
    if s in sdep:
        for _s in sdep[s]:
            dependencia = mdl.addConstrs((h[s,n] <= h[_s,n] for n in range(len(N))),name=("dep[%d,%d]"%(s,_s)))
            for idx in dependencia:
                dependencia[idx].setAttr("Lazy",1)
...
def cb(model,where):
    ...
    #                 print("servico %d na maq %d com constr. violada" %(s,m))
    #                 expr = quicksum(model._x[p,m] for p in S[s])
    #                 print(expr)
    #                 model.cbLazy(expr <= 1 )
...
if tex:
    ...
    print("\\verb|%s| & %g & %g & %g & %g & %g & %d & %s \\\\" % (name, all_time, load_time, mdl_vars, mdl_constrs, mdl_opt,resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024 +1 , output.decode("utf-8").strip('\n')))
