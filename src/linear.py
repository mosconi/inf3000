...
x = mdl.addVars(nproc,nmach,vtype=GRB.CONTINUOUS,lb=0,ub=1,name="x")
...
z_minus = mdl.addVars(nproc,nmach,name="z-",vtype=GRB.CONTINUOUS,lb=0,ub=1)
...
t = mdl.addVars(nmach,nmach,vtype=GRB.CONTINUOUS,lb=0,name="t")

if verbose: print("creating u",flush=True)
u = mdl.addVars(nmach,nres,name="u",vtype=GRB.CONTINUOUS,lb=0,ub=C)

if verbose: print("creating a",flush=True)
a = mdl.addVars(nmach,nres,name="a",vtype=GRB.CONTINUOUS,lb=0,ub=C)

if verbose: print("creating d",flush=True)
d = mdl.addVars(nmach,nres,name="d",vtype=GRB.CONTINUOUS,lb=0)

if verbose: print("creating b",flush=True)
b = mdl.addVars(nmach,nres,nres,vtype=GRB.CONTINUOUS,lb=0,name="b")

if verbose: print("creating o",flush=True)
o = mdl.addVars(nserv,len(L),vtype=GRB.CONTINUOUS,lb=0,ub=1,name="o")

if verbose: print("creating g",flush=True)
g = mdl.addVars(nserv,vtype=GRB.CONTINUOUS,lb=0,ub=1,name="s")

if verbose: print("creating h",flush=True)
h = mdl.addVars(nserv,len(N),vtype=GRB.CONTINUOUS,lb=0,ub=1,name="h")
...
loadcost = mdl.addVars(nres,name="locadcost",vtype=GRB.CONTINUOUS,obj=Wlc)
balancecost= mdl.addVars(nres,nres,name="balancecost",vtype=GRB.CONTINUOUS,obj=Wbal)
pmc = mdl.addVar(name="pmc",vtype=GRB.CONTINUOUS,lb=0,obj=WPMC)
smc = mdl.addVar(name="smc",vtype=GRB.CONTINUOUS,lb=0,obj=WSMC)
mmc = mdl.addVar(name="mmc",vtype=GRB.CONTINUOUS,lb=0,obj=WMMC)
...
if verbose: print("constr. t[i,j]=sum(y[p,i,j])")
...
if tex:
    import resource
    print("\\verb|%s| & %0.3f & %0.3f & %0.3f & %0.3f & %0.3f & %d & %0.3f  \\\\" % (name, all_time, load_time, mdl_vars, mdl_constrs, mdl_opt, resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024 +1, mdl.objVal ))
